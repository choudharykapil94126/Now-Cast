...
sub OnSignout()
    ' print "GridScreen.brs = > OnSignout"
...
sub signOutCancelBtnSelected()
    ' print "GridScreen.brs = > signOutCancelBtnSelected"

    m.signOutConfirmationBox.visible = false
    m.signOutCancelBtn.visible = false
    m.signOutConfirmBtn.visible = false
    m.global.vastTagUrlLogedInAndAgainCome = false
    m.global.vastTagUrlLogedInFirstTime = false
...
sub signOutConfirmBtnSelected()
    ' print "GridScreen.brs = > signOutConfirmBtnSelected"
...
sub onTimerEnd()
    ' print "GridScreen.brs: - onTimerEnd"
    m.bannerVideo.control = "play"
end sub

sub OnVisibleChange() ' invoked when GridScreen change visibility
    ' print "GridScreen.brs = > OnVisibleChange"

    if m.top.visible = true
        ' print "GridScreen.brs = > OnVisibleChange In If"
        ...
        ' print "GridScreen.brs = > OnVisibleChange In else"
...
sub OnItemFocused() ' invoked when another item is focused
    ' print "GridScreen.brs = > OnItemFocused"
    ' m.nowCastLogo.uri = m.global.organizationLogo
...
function OnkeyEvent(key as string, press as boolean) as boolean
    ' print "GridScreen.brs = > OnkeyEvent"
